...
class FmoeConformerLayer(nn.Module):
    ...
    def forward(
        ...
        aux_loss_macaron = None
        if self.feed_forward_macaron is not None:
            residual = x
            if self.normalize_before:
                x = self.norm_ff_macaron(x)
            h1 = self.feed_forward_macaron(x)
            x = residual + self.ff_scale * self.dropout(h1)
        ...
        h2, aux_loss = self.feed_forward(x, embed)
        x = residual + self.ff_scale * self.dropout(h2)
        if not self.normalize_before:
            x = self.norm_ff(x)
        ...
        return x, aux_loss_macaron, aux_loss, mask, new_cnn_cache
