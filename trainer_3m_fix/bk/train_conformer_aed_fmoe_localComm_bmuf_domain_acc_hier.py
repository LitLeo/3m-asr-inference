...
class LossCompute(nn.Module):
    ...
    def compute_loss(self, aux_loss, output, output_embed, lens, ctc_target,
                     ctc_lens, decoder_out, decoder_out_6, decoder_out_12, r_decoder_out, r_decoder_out_6, r_decoder_out_12, ys_out_pad, r_ys_out_pad, training=False):
        loss = 0.0
        metric, count = (), ()
        # CTC branch
        ...
        if self.ctc_weight != 1.0:
           ...
           ##### loss = loss + (1 - self.ctc_weight) * loss_att
            loss_att_6 = loss_att_6 * 0.1
            loss_att_12 = loss_att_12 * 0.1
            loss = loss + (1 - self.ctc_weight) * (loss_att + loss_att_6 + loss_att_12)
            loss = loss * 0.0001
            metric += metric_att
            count += count_att
        # embed_branch
...
########################                    miss_param_list += [k]
    ...
    def train_one_epoch(self, epoch):
        ...
        for (data, target, aed_target, lens, label_lens, aed_lens, key_strs) in \
            ...
            output, out_lens, decoder_out, decoder_out_6, decoder_out_12, ys_out_pad, r_decoder_out, r_decoder_out_6, r_decoder_out_12, r_ys_out_pad, output_embed, aux_loss = \
            ...
            metrics, counts = self.train_loss_obj.compute_loss(
                    ...
                    label_lens, decoder_out, decoder_out_6, decoder_out_12, r_decoder_out, r_decoder_out_6, r_decoder_out_12, ys_out_pad, r_ys_out_pad, training=True)
