...
class LossCompute(nn.Module):
    def __init__(self, padding_idx, blank_idx, loss_scale=1.0, loss_type='ctc'):
...
class Trainer(object):
    ...
    def make_loss_obj(self):
        ...
        loss_scale = self.cfg.get('loss_scale', 0.0)
        assert loss_type in ['ce', 'ctc']
        if loss_type == 'ce':
        ...
        self.train_loss_obj = LossCompute(padding_idx, blank_idx,
                loss_scale=loss_scale, loss_type=loss_type)
        self.valid_loss_obj = LossCompute(padding_idx, blank_idx,
                loss_scale=loss_scale, loss_type=loss_type)
    ...
    def train_one_epoch(self, epoch):
        ...
        # run data
        for (data, target, lens, label_lens, trans_batch) in \
            ...
            # write trans into file
            with open(fst_trans, 'w') as wf:
