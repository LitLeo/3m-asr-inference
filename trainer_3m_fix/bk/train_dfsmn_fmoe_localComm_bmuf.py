...
from loss.loss_compute import MetricStat, CELoss, CTCLoss, MoELayerScaleAuxLoss
...
class LossCompute(nn.Module):
    def __init__(self, padding_idx, blank_idx, embed_scale, num_aux,
                 aux_scale, auxloss_minimum, loss_type='ctc'):
    ...
    def compute_loss(self, aux_loss, output, output_embed, target,
                     lens, label_size, training=False):
        if self.loss_type == 'ce':
            main_loss, main_metric, main_count = self.criterion(output, target)
            if self.embed_scale > 0.0:
                embed_loss, embed_metric, embed_count = self.criterion(output_embed, target)
                main_loss = main_loss + self.embed_scale * embed_loss
                main_metric = main_metric + embed_metric
                main_count = main_count + embed_count
        else:
            main_loss, main_metric, main_count = self.criterion(output, lens, target, label_size)
            if self.embed_scale > 0.0:
                embed_loss, embed_metric, embed_count = self.criterion(output_embed, lens, target, label_size)
                main_loss = main_loss + self.embed_scale * embed_loss
                main_metric = main_metric + embed_metric
                main_count = main_count + embed_count
        aux_loss, aux_metric, aux_count = self.aux_criterion(aux_loss)
        loss = main_loss + aux_loss
        metric = main_metric + aux_metric
        count = main_count + aux_count
        if training:
            loss.backward()
        return metric, count
...
class Trainer(object):
    def __init__(self, args, cfg):
        ...
        self.cfg['model_conf']['moe_conf']['rank'] = self.local_rank
        self.cfg['model_conf']['moe_conf']['world_size'] = self.nproc_per_node
        assert self.nnodes > 1 and self.world_size > 1
        self.build_groups()
        mp_group = self.mp_groups[self.node_rank]
        dp_group = self.dp_groups[self.local_rank]
        self.cfg['model_conf']['moe_conf']['comm'] = mp_group
    ...
    def init_model_from_repeat(self, load_path):
        ...
        num_experts = self.cfg['model_conf']['moe_conf']['num_experts']
        world_size = self.cfg['model_conf']['moe_conf']['world_size']
        all_experts = num_experts * world_size
        assert all_experts >= load_experts_num
        rank = self.cfg['model_conf']['moe_conf']['rank']
    ...
    def make_loss_obj(self):
        ...
        if loss_type == 'ce':
            ...
            if embed_scale > 0.0:
                metric_tags += ['embed_ce_loss', 'embed_likely', 'embed_acc']
        ...
        aux_tags = ["aux_loss"]
        num_aux = 1
        router_regularization = self.cfg['model_conf']['moe_conf'] \
        ...
        if aux_scale is None:
            aux_scale = [0.1 for i in range(num_aux)]
        auxloss_minimum = self.cfg.get('auxloss_minimum', None)
        if auxloss_minimum is None:
            num_block = self.cfg['model_conf'].get('num_block', 3)
            fsmn_each_block = self.cfg['model_conf'].get('fsmn_each_block', 10)
            minimum = num_block * fsmn_each_block - 1
            auxloss_minimum = [minimum for i in range(num_aux)]
        self.train_metric = MetricStat(metric_tags)
        self.valid_metric = MetricStat(metric_tags)
        self.train_loss_obj = LossCompute(padding_idx, blank_idx, embed_scale,
                num_aux, aux_scale, auxloss_minimum, loss_type=loss_type)
        self.valid_loss_obj = LossCompute(padding_idx, blank_idx, embed_scale,
                num_aux, aux_scale, auxloss_minimum, loss_type=loss_type)
    ...
    def train_one_epoch(self, epoch):
        ...
        stick_router = self.cfg.get('stick_router', False)
        ...
        for (data, target, lens, label_lens) in self.train_loader(self.to_be_skip):
            ...
            output, output_embed, out_lens, aux_loss= self.model(
                    data, lens, is_training=True, stick_router=stick_router)
            metrics, counts = self.train_loss_obj.compute_loss(
                    aux_loss, output, output_embed, target, out_lens,
                    label_lens, training=True)
    ...
    def valid(self):
        ...
        for (data, target, lens, label_lens) in self.valid_loader(0):
            ...
            with torch.no_grad():
                output, output_embed, out_lens, aux_loss = self.model(
                        data, lens, is_training=False)
                metrics, counts = self.valid_loss_obj.compute_loss(
                        aux_loss, output, output_embed, target, out_lens,
                        label_lens, training=False)
