...
class DataLoader(object):
    ...
    def __call__(self, skip_num, output_keys=True):
    ...
    def produce(self, skip_num, output_keys):
        ...
        if done:
            ...
            return
        # data generation
        for (uttid, feat), label_ in zip(feat_reader, label_reader):
            ...
            if batch_idx == batch_size:
                ...
                if label is not None:
                    ...
                    aed_lens = None
                ###if not output_keys:
                ###    yield data, target, aed_target, data_lens, target_lens, aed_lens
                ###else:
                yield data, target, aed_target, data_lens, target_lens, aed_lens, copy(keys_batch)
                batch_idx = 0
                data_max_len = -1
                label_max_len = -1
                aed_max_len = -1
