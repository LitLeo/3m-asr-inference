...
class Net(nn.Module):
    ...
    def init_embed_model(self, load_path):
        ...
        model_dict = self.embed.state_dict()
        for k, v in param_dict.items():
            if k in model_dict and model_dict[k].size() == v.size():
               model_dict[k] = v
        self.embed.load_state_dict(model_dict)
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.blocks):
            ###if idx == 5:
            ###    xs_6, _, _, _, _ = layer(xs, embed, chunk_masks, pos_emb, mask_pad)
            ###elif idx == 11:
            ###    xs_12, _, _, _, _ = layer(xs, embed, chunk_masks, pos_emb, mask_pad)
            ###else:
