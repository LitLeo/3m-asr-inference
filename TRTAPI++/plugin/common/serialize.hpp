...
struct Serializer<std::vector<T>, typename std::enable_if<std::is_arithmetic<T>::value || std::is_enum<T>::value ||
  ...
  static size_t serialized_size(std::vector<T> const& value) { return sizeof(value.size()) + value.size() * sizeof(T); }
...
// serialize and deserialize value
template <typename T>
...
template <typename T>
inline size_t serialized_size(std::vector<T> const& value) {
  return Serializer<std::vector<T>>::serialized_size(value);
}

template <typename T>
inline void serialize_vector(void** buffer, std::vector<T> const& value) {
  return Serializer<std::vector<T>>::serialize(buffer, value);
}

template <typename T>
inline void deserialize_vector(void const** buffer, size_t* buffer_size, std::vector<T>* value) {
