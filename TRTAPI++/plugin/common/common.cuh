...
#ifndef PLUGIN_COMMON_COMMON_CUH_
#define PLUGIN_COMMON_COMMON_CUH_
...
template <typename T>
__device__ inline T sigmoid(const T& x);
...
template <>
__device__ inline half sigmoid(const half& x) {
#if __CUDA_ARCH__ >= 530
  const half h1 = __float2half(1.0f);
  return __hdiv(h1, __hadd(h1, exp(-x)));
#else
  return __float2half(1.0f / (exp(-__half2float(x)) + 1.0f));
#endif
}
...
__device__ inline void layerNorm(const kvp<R>& threadData, const int ld, const int offset, const P* beta,
  ...
  /*printf("%f %f %f \n", __half2float(sumKV.value), __half2float(mu), __half2float(rsigma));*/
  /*}*/
  ...
  for (int i = threadIdx.x; i < ld; i += TPB) {
    ...
    /*if (blockIdx.x == 0 && threadIdx.x == 0) {*/
    /*printf("%f %f %f %f %f\n", __half2float(gamma[0]), __half2float(beta[0]), __half2float(output[0]),
     * __half2float(mu), __half2float(rsigma));*/
    /*}*/
  }
}

template <typename T, typename P, int TPB>
__device__ inline void layerNormSmall(const T val, const kvp<T>& threadData, const int ld, const int idx, const P* beta,
                                      const P* gamma, T* output) {
  // Assuming threadData is already divided by ld
...
__device__ inline void scaledSoftmaxSmall(const int ld, const int lastValid, const float rsqrtHeadSize, const T* input,
                                          T* output) {
...
template <typename T, unsigned TPB>
__device__ inline void scaledSoftmax(const int ld, const int lastValid, const float rsqrtHeadSize, const T* input,
                                     T* output) {
  using BlockReduce = cub::BlockReduce<float, TPB>;
  __shared__ typename BlockReduce::TempStorage tmpStorage;
...
#endif  // PLUGIN_COMMON_COMMON_CUH_
