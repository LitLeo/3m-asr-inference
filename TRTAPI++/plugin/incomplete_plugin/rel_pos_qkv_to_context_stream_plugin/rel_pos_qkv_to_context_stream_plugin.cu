...
template <typename T>
__global__ void CatCacheKernel(const T* kvp, const T* cache, int B, 
                               int kvp_h, int cache_h, int w, int kvp_ldw, 
                               T* output) {
  int kvp_offset = 
}

inline void CatCache(const T* kvp, const T* cache, int B, int kvp_h, int cache_h, 
    int w, int kvp_ldw, T* output) {
  dim3 grid(kvp_h + cache_h, B, 1);
  dim3 block(256, 1, 1)
}
...
template<typename T>
int ComputeRelPosQKVToCtxTpl(QKVToContextStreamParams& params, const T* input, 
    const T* att_caches, const int* decode_frame_num, T* output) {
  // step1: cache ops

  // Q trans

  // trans_q and bias_u and bias_v

  // matrix_ac = tqu x trans_K' and matrix_bd = tqv x trans_P'

  // scores = (matrix_ac + matrix_bd) rsqrt + softmax

  // p_attn = scores x V

  // p_attn trans to output
}

int ComputeRelPosQKVToCtxStream(QKVToContextStreamParams & params, const float* input, float* output) {
...
int ComputeRelPosQKVToCtxStream(cublasHandle_t& cublas, const int B, const int S, const int num_heads, const int head_size,
    ...
    const int* maskIdx) {
  return ComputeRelPosQKVToCtxStreamTpl(cublas, B, S, num_heads, head_size, rsqrtHeadSize, input, output, qkptr, pptr, tptr, stream, maskIdx);
}
